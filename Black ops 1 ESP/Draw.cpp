...
void Draw::Draw3DBox(Vec3 Pos, ImColor color, int Thickness){
	...
	if (!w2s::WorldToScreen(tfl, tfl2D)) { return; }
	if (!w2s::WorldToScreen(tfr, tfr2D)) { return; }
	if (!w2s::WorldToScreen(tbl, tbl2D)) { return; }
	if (!w2s::WorldToScreen(tbr, tbr2D)) { return; }
	...
	if (!w2s::WorldToScreen(bfl, bfl2D)) { return; }
	if (!w2s::WorldToScreen(bfr, bfr2D)) { return; }
	if (!w2s::WorldToScreen(bbl, bbl2D)) { return; }
	if (!w2s::WorldToScreen(bbr, bbr2D)) { return; }
